...
- **Full session transcripts** *(v1.1+, controlled by the transcript toggle)* — a turn-by-turn record of each session: your prompts, the AI's text responses, and tool calls. Tool **results** are stored under a tiered policy to keep the substance without bloating disk: file Read/Edit/Write results are skipped (that content already lives in your files), Bash and WebFetch output is capped at 8 KB, results from other tools (e.g. MCP) are capped at 4 KB, and web-search/grep/glob results are kept in full. Wrapper artifacts that some agents inject into the prompt stream are filtered out at parse time. As an added safeguard, well-formed credentials that happen to appear in this text — API keys, access tokens, and private keys — are automatically replaced with a typed placeholder (e.g. `[redacted-api-key]`) before anything is stored, so secrets are not copied into the app's database or its search index. The original files on your disk are never modified.
...
Categorization runs automatically in the background in batches so the Insights tab populates without you waiting. It is on-device and uses the data already described above. If you correct a session's category yourself, that correction is stored locally, takes precedence over the automatic classification everywhere it's shown, and never leaves your Mac. Based on those corrections the app may suggest personal categorization rules (which you accept or ignore) and offers an **optional** export of an anonymized summary of your corrections — category labels and signal tags only, never your prompts or files — that you can choose to share with the developer. Nothing is ever sent automatically.
...
- **Settings → Clear Local Database** — wipes everything the app has stored locally *from your data*: usage records, parse cursors, cached authoritative cost data, the session-metadata digest (including your first/last prompt text), all captured transcripts and their search index, git correlation data, your daily reflection answers, any category corrections you've made, and any personal categorization rules. Your saved dashboards and app preferences are kept (those are configuration, not data). A re-scan rebuilds everything from the on-disk source files.
